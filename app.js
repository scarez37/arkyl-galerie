// enterGallery définie dans index.html (inline) pour être disponible immédiatement
        ...
        async function loadAdminDataFromServer() {
            try {
                ...
                const resp = await fetch(`/api_galerie_publique.php?admin=1&t=${Date.now()}`);
                ...
                for (const name of artistNames) {
                    ...
                    try {
                        const pResp = await fetch(`/api_modifier_profil.php?artist_name=${encodeURIComponent(name)}&t=${Date.now()}`);
                        if (pResp.ok) {
        ...
        async function handleArtistSpace() {
            ...
            try {
                const resp = await fetch('/auth.php?action=login_google', {
                    method: 'POST',
        ...
        async function chargerPanierUtilisateur(userId) {
            ...
            try {
                const response = await fetch(
                    `/api_get_panier.php?user_id=${encodeURIComponent(userId)}`
                );
        ...
        async function adminVoirProfilArtiste(name) {
            ...
            let artist = null;
            try {
                const resp = await fetch(`/api_modifier_profil.php?artist_name=${encodeURIComponent(name)}&t=${Date.now()}`);
                const result = await resp.json();
        ...
        async function renderAdminOverview() {
            ...
            try {
                const resp = await fetch('/api_commandes.php?action=list&admin=1');
        ...
        async function deleteAdminArtwork(id) {
            ...
            try {
                ...
                const resp = await fetch('/api_supprimer_oeuvre.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        async function toggleFavorite(event, productId) {
            ...
            try {
                ...
                const response = await fetch('/api_ajouter_favoris.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        async function addToCart(event, productId) {
            ...
            try {
                const response = await fetch('/api_ajouter_panier.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        async function renderFavorites() {
            ...
            try {
                const resp = await fetch(
                    `/api_get_favoris.php?user_id=${encodeURIComponent(userId)}&t=${Date.now()}`
                );
                const data = await resp.json();
        ...
        async function verifierPanierVsVendus() {
            ...
            try {
                const resp = await fetch(`/api_galerie_publique.php?ids=${ids.join(',')}`);
                const data = await resp.json();
        ...
        function changeQuantity(itemId, change) {
            ...
            if (_userId) {
                fetch('/api_supprimer_panier.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        async function viewArtistDetail(event, artistName) {
            ...
            try {
                const profileResp = await fetch(`/api_modifier_profil.php?artist_name=${encodeURIComponent(artistName)}&t=${Date.now()}`);
                if (profileResp.ok) {
        ...
        async function followArtist(artistName) {
            ...
            let allProducts = [];
            try {
                const response = await fetch('/api_galerie_publique.php?include_sold=1&t=' + Date.now());
        ...
        function _injectMonEspaceTab() {
            ...
            if (window._artworksCache && (now - window._artworksCache.ts) < ARTWORKS_CACHE_TTL) {
                ...
                try {
                    const response = await fetch('/api_galerie_publique.php?include_sold=1');
                    const contentType = response.headers.get('content-type');
                    if (response.ok && contentType && contentType.includes('application/json')) {
                        const result = await response.json();
                        if (result.success && result.data && result.data.length > 0) {
        ...
        async function goToArtistSpace() {
            ...
            if (currentUser && (currentUser.id || currentUser.googleId)) {
                const google_id = String(currentUser.id || currentUser.googleId);
                try {
                    const r = await fetch('/auth.php?action=login_google', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
        ...
        // ⭐ Pagination des œuvres artiste (évite de charger >1MB d'un coup)
        const _ARTWORKS_PER_PAGE = 20;
        let _artworksOffset = 0;
        let _artworksHasMore = true;
        ...
        async function loadArtistArtworksFromServer(reset = true) {
            ...
            if (reset) {
                _artworksOffset = 0;
                _artworksHasMore = true;
                db.artworks = [];
            }

            if (!_artworksHasMore || _artworksLoading) return;

            // Afficher le skeleton pendant le chargement
            _artworksLoading = true;
            if (reset) showSkeletonLoader('artworksGrid', 6, 'grid');

            try {
                const url = `/api_galerie_publique.php`
                ...
                if (result.success && result.data && result.data.length > 0) {
                    ...
                    const newArtworks = result.data.map(art => ({
                        id:          art.id,
                        server_id:   art.id,
                        title:       art.title,
                        category:    art.category,
                        ...
                        description: art.description || '',
                        photo:       art.image_url,
                        photos:      [art.image_url].filter(Boolean), // URL unique, pas de tableau lourd
                    ...
                    db.artworks = reset ? newArtworks : [...db.artworks, ...newArtworks];
                    _artworksOffset += result.data.length;

                    // Plus de pages si on reçoit moins que la limite demandée
                    if (result.data.length < _ARTWORKS_PER_PAGE) {
                    ...
                    if (db.artworks.length === 0) {
                        ...
                        if (_artworksRetryCount < _ARTWORKS_MAX_RETRY) {
                            ...
                            setTimeout(() => loadArtistArtworksFromServer(false), delai);
                        } else {
        ...
        // Charger la page suivante d'œuvres (appelable depuis un bouton "Voir plus")
        async function loadMoreArtistArtworks() {
            await loadArtistArtworksFromServer(false);
        }
        ...
        async function updateDashboard() {
            ...
            try {
                ...
                const resp = await fetch(`/api_commandes.php?action=list&artist_id=${artistId}`);
                const json = await resp.json();
            ...
            try {
                ...
                if (editingArtworkId) {
                    ...
                    try {
                        ...
                        const resp = await fetch('/api_modifier_oeuvre.php', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                    ...
                    try {
                        ...
                        const response = await fetch('/api_ajouter_oeuvre.php', {
                            method: 'POST',
                            headers: {
        ...
        async function saveArtistProfile() {
            ...
            try {
                ...
                try {
                    ...
                    const resp = await fetch('/api_modifier_profil.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
   ...
   ============================ */
    ...
    async function viewProductDetailFromAPI(artworkId) {
        ...
        try {
            ...
            const response = await fetch(`/api_galerie_publique.php?artwork_id=${artworkId}`);
            const result = await response.json();
    ...
    async function _revaliderEnArrierePlan() {
        try {
            const urlAPI = `/api_galerie_publique.php?limit=${ITEMS_PER_LOAD}&offset=0&t=${Date.now()}`;
    ...
    async function chargerPlusOeuvres() {
        ...
        try {
            const urlAPI = `/api_galerie_publique.php?limit=${ITEMS_PER_LOAD}&offset=${currentOffset}&t=${Date.now()}`;
    ...
    draw();
        ...
        async function chargerTresorerieAdmin() {
            try {
                ...
                const response = await fetch('/api_commandes.php?action=list&admin=1');
