...
// 🔧 DÉDOUBLONNER les bases trouvées (au cas où même fichier trouvé dans plusieurs dossiers)
$uniqueDatabases = [];
$seenPaths = [];
foreach ($foundDatabases as $db) {
    $realPath = realpath($db['path']);
    if ($realPath && !in_array($realPath, $seenPaths)) {
        $seenPaths[] = $realPath;
        $db['path'] = $realPath; // Utiliser le chemin réel
        $uniqueDatabases[] = $db;
    }
}
$foundDatabases = $uniqueDatabases;
...
if (count($foundDatabases) == 1) {
    ...
    // Vérifier si elle a les bonnes colonnes
    if (!$foundDatabases[0]['has_new_columns']) {
        echo "<p class='warning'>⚠️ Cette base n'a pas les nouvelles colonnes (width, technique, etc.)</p>";
        echo "<p>Lance maintenant : <a href='/migrate_db_v2.php' style='color:#0ff;'>migrate_db_v2.php</a></p>";
    } else {
        echo "<p class='success'>✅ Cette base a déjà toutes les colonnes nécessaires !</p>";
        echo "<p>Tu peux maintenant utiliser <a href='/test_quick.php' style='color:#0ff;'>test_quick.php</a> pour remplir les données.</p>";
    }
    exit;
}
